...
/**
 * Created by isansmith on 16/1/20.
 */
public class SlidebarView extends View {
    ...
    public SlidebarView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void setIndex(String[] strings){
        A_Z = strings;
        this.invalidate();
    }
...
//        int singleHeight = height / A_Z.length - 2;// 获取每一个字母的高度 (这里-2仅仅是为了好看而已)
        singleHeight = (int)(getResources().getDisplayMetrics().density * 20 + 0.5f);
...
//            float yPos = singleHeight * i + singleHeight;
            float yPos = singleHeight * i + singleHeight + (height - singleHeight * A_Z.length)/2;
...
//        final int c = (int) (y / getHeight() * A_Z.length);// 点击y坐标所占总高度的比例*b数组的长度就等于点击b中的个数.
        final int c = (int) (y -(height - singleHeight * A_Z.length)/2) / singleHeight;
        Log.i("logger----","  y:"+y+"      "+(height - singleHeight * A_Z.length)/2+"    "+c);
